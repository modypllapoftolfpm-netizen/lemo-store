...
export default function Home() {
  ...
  const c = { p: settings.primaryColor || "#C9A96E", d: settings.darkColor || "#3D2B1F", bg: settings.bgColor || "#FAF7F2" };
    ...
    <div style={{ minHeight: "100vh", background: c.bg, fontFamily: lang === "ar" ? "Cairo, sans-serif" : "DM Sans, sans-serif" }}>
      ...
      {banners.length > 0 ? (
        <div style={{ position: "relative", height: "520px", overflow: "hidden" }}>
          ...
          <div style={{ position: "absolute", inset: 0, background: `linear-gradient(to bottom, rgba(0,0,0,0.1), ${c.d}CC)`, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", paddingBottom: "4rem", color: "#fff", textAlign: "center" }}>
            <h1 style={{ fontSize: "3.5rem", fontWeight: "800", margin: "0 0 1rem", textShadow: "0 2px 10px rgba(0,0,0,0.3)" }}>{banners[currentBanner]?.titleAr || "🕯️ LEMO Store"}</h1>
            <p style={{ fontSize: "1.2rem", opacity: 0.9, marginBottom: "2rem", maxWidth: "500px" }}>{banners[currentBanner]?.subtitleAr || "شموع فاخرة وهدايا مميزة"}</p>
          ...
          {banners.length > 1 && (
            <div style={{ position: "absolute", bottom: "20px", left: "50%", transform: "translateX(-50%)", display: "flex", gap: "8px" }}>
              {banners.map((_, i) => <button key={i} onClick={() => setCurrentBanner(i)} style={{ width: i === currentBanner ? "28px" : "8px", height: "8px", borderRadius: "4px", background: i === currentBanner ? c.p : "rgba(255,255,255,0.6)", border: "none", cursor: "pointer", transition: "all 0.3s" }} />)}
            </div>
          )}
        </div>
      ) : (
...
function ProductCard({ product, field, t, addToCart, c }) {
  ...
  return (
    <div style={{ background: "#fff", borderRadius: "20px", boxShadow: "0 4px 20px rgba(0,0,0,0.06)", width: "230px", overflow: "hidden", transition: "transform 0.3s, box-shadow 0.3s" }} onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-8px)"; e.currentTarget.style.boxShadow = `0 15px 40px ${c.p}33`; }} onMouseLeave={(e) => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 4px 20px rgba(0,0,0,0.06)"; }}>
      <Link to={`/products/${product.id}`} style={{ textDecoration: "none" }}>
        <div style={{ height: "220px", background: c.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "4rem", position: "relative", overflow: "hidden" }}>
          {product.imageUrl ? <img src={product.imageUrl} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "🕯️"}
          {product.isNew && <span style={{ position: "absolute", top: "12px", right: "12px", background: `linear-gradient(135deg, ${c.p}, ${c.p}DD)`, color: "#fff", padding: "4px 12px", borderRadius: "12px", fontSize: "0.75rem", fontWeight: "700" }}>جديد</span>}
        ...
        <div style={{ padding: "1rem 1.2rem 0.5rem" }}>
          <h3 style={{ margin: "0 0 6px", color: c.d, fontSize: "1rem", fontWeight: "700" }}>{field(product, "name")}</h3>
