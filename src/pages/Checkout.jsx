...
export default function Checkout() {
  ...
  // تأمين الأرقام عشان مشكلة الـ NaN
  ...
  const handleOrder = async (e) => {
    ...
    try {
      ...
      clearCart();
      // التوجه لخطوة الواتساب بدلاً من الخروج من الصفحة
      setStep(3); 
    } catch (err) {
      console.error(err);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FAF8F5", fontFamily: "'Cairo', sans-serif" }} dir="rtl">
      ...
      <div style={{ maxWidth: "900px", margin: "2rem auto", padding: "0 1.5rem" }}>
        ...
        {/* إخفاء العنوان والخطوات لو وصلنا لخطوة الواتساب الأخيرة */}
        {step < 3 && (
          <>
            <h1 style={{ color: "#3D2B1F", marginBottom: "2rem", fontSize: "2rem", fontWeight: "900" }}>💳 إتمام الطلب</h1>
            
            {/* شريط الخطوات (Steps) بتصميم فخم */}
            <div style={{ display: "flex", marginBottom: "2.5rem", background: "#fff", borderRadius: "16px", overflow: "hidden", boxShadow: "0 4px 15px rgba(0,0,0,0.03)" }}>
              {["بيانات التوصيل", "تأكيد الطلب"].map((s, i) => (
                <div key={i} style={{ 
                  ...
                  background: step === i + 1 ? "linear-gradient(135deg, #C9A96E, #b8925a)" : step > i + 1 ? "#FAF8F5" : "#fff", 
                  color: step === i + 1 ? "#fff" : step > i + 1 ? "#C9A96E" : "#A89A8E", 
                  fontWeight: "800", fontSize: "1rem", transition: "all 0.4s" 
                }}>
                  {step > i + 1 ? "✓" : `${i + 1}.`} {s}
                </div>
              ))}
        ...
        <div style={{ display: "flex", gap: "2rem", flexWrap: "wrap", flexDirection: step === 3 ? "column" : "row" }}>
            ...
            <div style={{ flex: 2, minWidth: "300px" }}>
              ...
              {step === 1 && (
                <div style={{ background: "#fff", borderRadius: "20px", padding: "2rem", boxShadow: "0 10px 40px rgba(61,43,31,0.05)", border: "1px solid #E8DDD0" }}>
                  ...
                  <button 
                    onClick={() => { if (!form.name || !form.phone || !form.address) { alert("برجاء ملء كافة البيانات المطلوبة لتوصيل طلبك."); return; } setStep(2); }} 
                    style={{ width: "100%", background: "#3D2B1F", color: "#fff", border: "none", borderRadius: "12px", padding: "16px", fontSize: "1.1rem", fontWeight: "800", cursor: "pointer", marginTop: "1rem", transition: "0.3s" }}
                  >
                    متابعة لتأكيد الطلب ←
                  </button>
                </div>
              ...
              {step === 2 && (
                <div style={{ background: "#fff", borderRadius: "20px", padding: "2rem", boxShadow: "0 10px 40px rgba(61,43,31,0.05)", border: "1px solid #E8DDD0", animation: "fadeIn 0.4s" }}>
                  ...
                  <div style={{ display: "flex", gap: "12px", marginTop: "2rem" }}>
                    <button onClick={() => setStep(1)} style={{ flex: 1, background: "#fff", border: "2px solid #E8DDD0", color: "#3D2B1F", borderRadius: "12px", padding: "14px", cursor: "pointer", fontWeight: "700", fontSize: "1rem" }}>
                      تعديل البيانات
                    </button>
          ...
          {/* الجانب الأيسر (ملخص السلة) - يختفي في الخطوة 3 */}
          {step < 3 && (
            <div style={{ flex: 1, minWidth: "280px" }}>
              <div style={{ background: "#fff", borderRadius: "20px", padding: "2rem", boxShadow: "0 10px 40px rgba(61,43,31,0.05)", border: "1px solid #E8DDD0", position: "sticky", top: "20px" }}>
                <h3 style={{ color: "#3D2B1F", marginBottom: "1.5rem", fontSize: "1.2rem", fontWeight: "800", display: "flex", alignItems: "center", gap: "8px" }}>
                  <span>🛒</span> ملخص طلبك
                </h3>
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      <div key={item.id} style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "15px", paddingBottom: "15px", borderBottom: "1px solid #FAF8F5" }}>
                        {item.image ? (
                           <img src={item.image} style={{ width: "60px", height: "60px", borderRadius: "10px", objectFit: "cover", border: "1px solid #F0E8DF" }} alt={item.nameAr} />
                        ) : (
                           <div style={{ width: "60px", height: "60px", borderRadius: "10px", background: "#FAF8F5", display: "flex", alignItems: "center", justifyContent: "center" }}>🕯️</div>
                        )}
                        <div style={{ flex: 1 }}>
                ...
                <div style={{ background: "#FAF8F5", padding: "1.2rem", borderRadius: "12px", marginTop: "1rem" }}>
                  {discount > 0 && (
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px", color: "#E74C3C", fontWeight: "700", fontSize: "0.95rem" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "900", fontSize: "1.3rem", color: "#3D2B1F", borderTop: discount > 0 ? "1px solid #E8DDD0" : "none", paddingTop: discount > 0 ? "8px" : "0" }}>
                    <span>الإجمالي</span>
                    <span style={{ color: "#C9A96E" }}>{safeTotal.toFixed(0)} ج.م</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* الخطوة 3: رسالة النجاح والتحويل للواتساب */}
          {step === 3 && (
            <div style={{ background: "#fff", borderRadius: "24px", padding: "4rem 2rem", boxShadow: "0 15px 50px rgba(61,43,31,0.08)", border: "1px solid #E8DDD0", textAlign: "center", maxWidth: "650px", margin: "0 auto", animation: "fadeIn 0.6s ease-out" }}>
              ...
              <h2 style={{ color: "#3D2B1F", marginBottom: "1.5rem", fontSize: "2.2rem", fontWeight: "900" }}>
                تم تسجيل طلبك بنجاح!
              </h2>
              ...
              <p style={{ color: "#555", fontSize: "1.2rem", lineHeight: "1.9", marginBottom: "3rem", fontWeight: "600" }}>
                لأن كل قطعة في <span style={{ color: "#3D2B1F", fontWeight: "900" }}>Lemo Store</span> صنعت خصيصاً لك بلمسة فنية مميزة، 
                <span style={{ color: "#C9A96E" }}> يسعدنا تواصلك معنا </span> 
                لتنسيق كافة التفاصيل وضمان أن طلبك سيكون تماماً كما تخيلته. تواصل معنا الآن لإتمام اللمسات الأخيرة لطلبك.
              </p>
              
              <a href={`https://wa.me/201009633100?text=أهلاً LEMO، قمت بتسجيل طلب باسم: ${form.name} وأريد استكمال اللمسات الأخيرة لطلبي.`} 
                 target="_blank" 
                 rel="noopener noreferrer"
                 style={{
                   display: "inline-flex", alignItems: "center", justifyContent: "center", gap: "12px",
                   background: "#25D366", color: "#fff", padding: "18px 40px", borderRadius: "50px",
                   textDecoration: "none", fontSize: "1.2rem", fontWeight: "800",
                   boxShadow: "0 8px 25px rgba(37,211,102,0.35)", transition: "0.3s"
                 }}
