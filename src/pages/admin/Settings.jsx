      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "2rem" }}>
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "2rem", flexWrap: "wrap" }}>
          {tabs.map((tab) => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{
              ...
              background: activeTab === tab.key ? "#C9A96E" : "#fff",
              color: activeTab === tab.key ? "#fff" : "#C9A96E",
            }}>{tab.label}</button>
          ))}
        </div>
        ...
        <form onSubmit={handleSave}>
          <div style={{ background: "#fff", borderRadius: "16px", padding: "2rem", boxShadow: "0 2px 10px rgba(0,0,0,0.05)" }}>
            ...
            {activeTab === "home" && (
              <div>
                ...
                <label style={{ display: "block", marginBottom: "6px", color: "#3D2B1F", fontWeight: "600" }}>عنوان البانر (عربي)</label>
                <input name="heroTitleAr" value={settings.heroTitleAr} onChange={handleChange} style={inputStyle} />
                <label style={{ display: "block", marginBottom: "6px", color: "#3D2B1F", fontWeight: "600" }}>عنوان البانر (إنجليزي)</label>
                <input name="heroTitleEn" value={settings.heroTitleEn} onChange={handleChange} style={inputStyle} />
                <label style={{ display: "block", marginBottom: "6px", color: "#3D2B1F", fontWeight: "600" }}>النص الفرعي (عربي)</label>
                <input name="heroSubtitleAr" value={settings.heroSubtitleAr} onChange={handleChange} style={inputStyle} />
                <label style={{ display: "block", marginBottom: "6px", color: "#3D2B1F", fontWeight: "600" }}>النص الفرعي (إنجليزي)</label>
                <input name="heroSubtitleEn" value={settings.heroSubtitleEn} onChange={handleChange} style={inputStyle} />
                <label style={{ display: "block", marginBottom: "6px", color: "#3D2B1F", fontWeight: "600" }}>عنوان قسم الأكثر مبيعاً</label>
                <input name="bestSellersTitle" value={settings.bestSellersTitle} onChange={handleChange} style={inputStyle} />
                <label style={{ display: "block", marginBottom: "6px", color: "#3D2B1F", fontWeight: "600" }}>عنوان قسم وصل حديثاً</label>
                <input name="newArrivalsTitle" value={settings.newArrivalsTitle} onChange={handleChange} style={inputStyle} />
              </div>
            )}
              ...
              <div>
                ...
                {[
                  ...
                  <div key={c.name} style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px" }}>
                    <input type="color" name={c.name} value={settings[c.name]} onChange={handleChange}
          ...
          <button type="submit" disabled={loading} style={{
            ...
            background: saved ? "#4CAF50" : "#C9A96E",
