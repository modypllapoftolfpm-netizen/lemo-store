...
const MovingCategoryTicker = ({ categories, field }) => (
  <div style={{ background: "#111", color: "#fff", padding: "12px 0", overflow: "hidden", whiteSpace: "nowrap", width: "100%", fontSize: "0.9rem", fontWeight: "600", borderBottom: "1px solid #333", margin: "2rem 0" }}>
    <div style={{ display: "inline-block", animation: "marquee 25s linear infinite", paddingLeft: "100%" }}>
      {categories.concat(categories).map((cat, index) => (
        <span key={index} style={{ margin: "0 25px", textTransform: "uppercase", letterSpacing: "1px" }}>{field(cat, "name")} ✦</span>
      ))}
    </div>
...
export default function Home() {
  ...
  const [settings, setSettings] = useState({}); // <--- إضافة الإعدادات
  ...
  useEffect(() => {
    ...
    const unsubProds = subscribeToProducts((data) => {
      console.log("المنتجات القادمة من Firebase:", data);
      setProducts(data);
    });
    
    // جلب الإعدادات وحفظها
  ...
  const bestSellers = products.filter(p => p.bestSeller === true || p.bestSeller === "true");
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FAF8F5", fontFamily: "'Cairo', sans-serif" }} dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <MovingCategoryTicker categories={categories} field={field} />
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "2rem", flexWrap: "wrap" }}>
          {bestSellers.length > 0 ? bestSellers.map(product => (
            <Link key={product.id} to={`/product/${product.id}`} style={{ textDecoration: "none", color: "inherit", width: "250px" }}>
              ...
              {/* تفعيل ظهور المخزون بناءً على إعدادات الموقع */}
              {settings?.showStock !== false && (
                <p style={{ fontSize: "0.85rem", color: "#777", marginTop: "5px" }}>المتبقي: {product.stock}</p>
              )}
            </Link>
