...
const CATS = [
  { key: "gifts", labelAr: "الهدايا", labelEn: "Gifts", img: "https://images.unsplash.com/photo-1549465220-1a8b9238cd48?w=400" },
  { key: "scented", labelAr: "الشموع المعطرة", labelEn: "Scented Candles", img: "https://images.unsplash.com/photo-1603006905003-be475563bc59?w=400" },
  { key: "decorative", labelAr: "الشموع الديكورية", labelEn: "Decorative", img: "https://images.unsplash.com/photo-1602874801007-bd458bb1b8b6?w=400" },
  { key: "body", labelAr: "مرطبات الجسم", labelEn: "Body Care", img: "https://images.unsplash.com/photo-1608248543803-ba4f8c70ae0b?w=400" },
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    return () => { unsubBanners(); unsubProds(); };
  }, []);
  ...
  const ProductCard = ({ product }) => (
    <Link to={`/products/${product.id}`} style={{ textDecoration: "none", color: "inherit", width: "220px" }}>
      <div style={{ borderRadius: "16px", overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.08)", background: "#fff", transition: "transform 0.3s" }}
        ...
        <img src={Array.isArray(product.imageUrl) ? product.imageUrl[0] : product.imageUrl} style={{ width: "100%", height: "220px", objectFit: "cover" }} alt="" />
    ...
    <div style={{ minHeight: "100vh", background: "#FAF8F5", fontFamily: "'Cairo', sans-serif" }} dir={lang === "ar" ? "rtl" : "ltr"}>
      ...
      <div style={{ padding: "4rem 2rem", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "2.5rem", flexWrap: "wrap" }}>
          {CATS.map((cat) => (
            <Link key={cat.key} to={`/products?category=${cat.key}`} style={{ textDecoration: "none", color: "inherit" }}>
              <div style={{ transition: "transform 0.3s" }}
                onMouseEnter={e => e.currentTarget.style.transform = "translateY(-8px)"}
                onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}>
                <div style={{ width: "160px", height: "160px", borderRadius: "50%", overflow: "hidden", border: "3px solid #E8DDD0", boxShadow: "0 5px 20px rgba(0,0,0,0.1)", margin: "0 auto" }}>
                  <img src={cat.img} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" />
                </div>
                <h3 style={{ marginTop: "1rem", color: "#111", fontWeight: "700", fontSize: "1rem" }}>
      ...
      {/* زرار تواصل ثابت أسفل يسار */}
      <a href={`https://wa.me/${settings?.whatsapp || "201009633100"}`} target="_blank" rel="noreferrer"
        style={{ position: "fixed", bottom: "24px", left: "24px", background: "#111", color: "#fff", padding: "12px 24px", borderRadius: "50px", textDecoration: "none", fontWeight: "700", fontSize: "0.95rem", boxShadow: "0 4px 20px rgba(0,0,0,0.3)", zIndex: 1000, display: "flex", alignItems: "center", gap: "8px" }}>
        💬 تواصل معنا
      </a>
    </div>
  );
